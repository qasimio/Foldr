...
Routing
-------
Subcommands (watch, undo, history, config) are detected by scanning ALL
positional args BEFORE argparse runs. This fixes the bug where
`foldr ~/Downloads/Documents undo` would not trigger the undo command.

Mode system
-----------
User picks mode once (`foldr config --mode tui|cli`) → saved to
~/.foldr/prefs.json → applied every run. Override per-run with --plain.

Undo design (non-sequential, independent)
-----------------------------------------
Each foldr operation writes one history entry. Any entry can be undone
independently — you do NOT need to undo newer operations first.
If a file from op A was later moved by op B, undoing op A will skip
that file and warn you, rather than blindly clobbering op B's result.

Flags (user-friendly names)
---------------------------
  --preview         was --dry-run
  --depth N         was --max-depth
  --follow-links    was --follow-symlinks
  --dedup STRAT     was --deduplicate
  --global-ignore   use ~/.foldr/.foldrignore on top of local .foldrignore
  --plain           force CLI output this run (no TUI)
"""
...
def _ok(msg:str)   -> None:
    if not _QUIET: print(f"  {COL_OK}✓{RESET}  {msg}")
def _warn(msg:str) -> None:
    if not _QUIET: print(f"  {COL_WARN}!{RESET}  {msg}")
def _err(msg:str)  -> None:
    print(f"  {COL_ERR}✗{RESET}  {msg}", file=sys.stderr)
def _dim(msg:str)  -> None:
    if not _QUIET: print(f"  {FG_MUTED}{msg}{RESET}")
def _info(msg:str) -> None:
    if not _QUIET: print(f"  {ACCENT}·{RESET}  {msg}")

def _summary(cat_counts:dict, moved:int, other:int,
             ignored:int, elapsed:float, dry:bool) -> None:
    if _QUIET or not cat_counts: return
    w     = _w(); bar_w = max(12, w // 3)
...
def _confirm(
    ...
    danger: bool = False,
    plain_prompt: str = "",
    plain_default: bool = False,
) -> bool:
    if _USE_TUI:
        from foldr.tui import confirm_dialog, Screen as _Scr
        scr = _Scr(); scr.enter()
        try:
            return confirm_dialog(scr, title, body_lines, yes_label=yes,
                                  no_label=no, danger=danger)
        finally:
            scr.exit()
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples\n"
            "--------\n"
            "  foldr ~/Downloads                     organize with preview first\n"
            "  foldr ~/Downloads --preview            show what would happen, no changes\n"
            "  foldr ~/Downloads --recursive          include files in subfolders\n"
            "  foldr ~/Downloads --depth 3            limit subfolder depth\n"
            "  foldr ~/Downloads --dedup keep-newest  remove duplicate files\n"
            "  foldr ~/Downloads --ignore '*.log'     skip .log files\n"
            "  foldr ~/Downloads --global-ignore      also use ~/.foldr/.foldrignore\n"
            "  foldr ~/Downloads --smart              detect misnamed files by content\n"
            "\n"
            "  foldr watch ~/Downloads                auto-organize new files as they arrive\n"
            ...
            "  foldr config --mode tui                switch to TUI mode permanently\n"
            "  foldr config --mode cli                switch to CLI mode permanently\n"
        ),
    )
    ...
    p.add_argument("--depth",        type=int,    metavar="N",
                   help="Maximum subfolder depth (use with --recursive)")
    ...
    p.add_argument("--smart",        action="store_true",
                   help="Detect file type by content, not just extension (catches misnamed files)")
    p.add_argument("--dedup",
                   choices=["keep-newest", "keep-largest", "keep-oldest"],
                   metavar="STRATEGY",
                   help="Find and remove duplicate files. STRATEGY: keep-newest | keep-largest | keep-oldest")
    p.add_argument("--ignore",       nargs="+",   metavar="PATTERN",
                   help="Skip files matching these patterns, e.g. '*.log' 'tmp/'")
    p.add_argument("--global-ignore",action="store_true",
                   help="Also apply rules from ~/.foldr/.foldrignore")
    p.add_argument("--config",       metavar="FILE",
                   help="Use a custom category config file (.toml)")
    p.add_argument("--verbose",      action="store_true",
                   help="Print each file that gets moved")
    p.add_argument("--quiet",        action="store_true",
                   help="Suppress all output (for scripts)")
    p.add_argument("--plain",        action="store_true",
                   help="Use plain text output for this run (no TUI)")
...
def cmd_watch(raw_argv: list[str], args) -> None:
    # Extract the directory that follows 'watch'
    try:
    ...
    _info(f"Watching {target}")
    _dim("New files dropped here will be organized automatically.")
    _dim("Press Ctrl+C to stop.")
    print()

    from foldr.watch import run_watch
    run_watch(base=target, template=tmpl or {}, dry_run=args.preview,
              extra_ignore=ignore, use_tui=_USE_TUI)

# ── undo ───────────────────────────────────────────────────────────────────────
def cmd_undo(args) -> None:
    _rule("Undo")

    # Resolve which history entry to undo
    target_id = getattr(args, "id", None)
    if target_id:
        log = get_history_entry(target_id)
        if not log:
            _err(f"No history entry found for ID: {target_id}")
            _dim("Run 'foldr history' to see all operation IDs.")
            return
    else:
    ...
    # Show summary of what we're about to undo
    _box(
    ...
    if otype == "dedup":
        ...
        _dim("Tip: always run 'foldr --dedup ... --preview' first.")
        return
    ...
    if not dry:
        ok = _confirm(
            ...
            body_lines=[
                "",
                f"  Restore {ACCENT+BOLD}{total}{RESET} files",
                f"  from operation {ACCENT}{eid}{RESET}",
                f"  in {FG_DIM}{Path(base).name}/{RESET}",
                "",
                f"  {FG_MUTED}Files moved elsewhere since will be skipped, not overwritten.{RESET}",
                "",
            ],
            yes=" Undo ", no=" Cancel ",
            danger=True,
    ...
    # Output — CLI style always (undo output is a list, not a TUI screen)
    print()
    _rule("Restored" if not dry else "Would Restore")
    print()
    ...
    print()
    _box(
        f"  {COL_OK+BOLD}{len(result.restored)}{RESET} restored  "
        f"{FG_MUTED}·{RESET}  "
        f"{COL_WARN}{len(result.skipped)}{RESET} skipped  "
        f"{FG_MUTED}·{RESET}  "
...
def cmd_history(args) -> None:
    ...
    try:
        ...
        for e in entries:
            ...
            rows.append([e.get("id","?"), ot, ts, base, str(e.get("total_files",0))])
        ...
        for e in entries:
            ...
            ot  = e.get("op_type","organize")
            oi  = op_icon(ot)
            print(f"  {ACCENT}{e.get('id','?')}{RESET}  {FG_MUTED}{oi} {ot:<10}{RESET}"
                  f"  {ts}  {Path(e.get('base','?')).name}"
                  f"  {FG_DIM}{e.get('total_files',0)} files{RESET}")
    print()
    _dim("To undo an operation: foldr undo --id <ID>")

# ── config ─────────────────────────────────────────────────────────────────────
def cmd_config(raw_argv: list[str]) -> None:
    mode = None
    for i, a in enumerate(raw_argv):
        if a == "--mode" and i+1 < len(raw_argv):
            mode = raw_argv[i+1]; break
        if a.startswith("--mode="):
            mode = a.split("=",1)[1]; break

    if mode:
        if mode not in ("tui", "cli"):
            _err(f"Unknown mode '{mode}'. Use: tui or cli"); sys.exit(1)
        set_mode(mode)
        _ok(f"Mode set to {ACCENT+BOLD}{mode}{RESET}")
        _dim("Saved to ~/.foldr/prefs.json")
    else:
        ...
        print()
        _dim("Change mode: foldr config --mode tui|cli")
...
def cmd_dedup(target: Path, strat_str: str, recursive: bool,
    ...
    _info(f"Scanning {target} for duplicates …")
    ...
    _warn(f"Found {len(groups)} groups — {total_rem} removable duplicate files")
    ...
    try:
        from tabulate import tabulate
        rows = [
            [g.keep.name if g.keep else "?", rem.name,
             fmt_size(g.keep.stat().st_size if g.keep and g.keep.exists() else 0)]
            for g in groups[:40] for rem in g.remove
        ]
        print("\n" + tabulate(rows, headers=["Keep","Remove","Size"],
                              tablefmt="rounded_outline"))
    except ImportError:
        for g in groups[:20]:
            for rem in g.remove:
                print(f"  {COL_OK}keep:{RESET} {g.keep.name if g.keep else '?'}"
                      f"  {COL_ERR}del:{RESET} {rem.name}")
    ...
    ok = _confirm(
        ...
        body_lines=[
            "",
            f"  Delete {COL_ERR+BOLD}{total_rem}{RESET} duplicate files?",
            f"  Strategy: {ACCENT}{strat_str}{RESET}",
            "",
            f"  {COL_WARN}Dedup cannot be reversed with 'foldr undo'.{RESET}",
            "",
        ],
    ...
    save_dedup_history(removed_paths, target, strat_str)
    _ok(f"Removed {len(removed_paths)} duplicate files.")
    _dim("Note: dedup history is recorded but files cannot be restored via undo.")
...
def cmd_organize(target: Path, args, template) -> None:
    ...
    if not prev.actions:
        _box(f"  {COL_OK+BOLD}Nothing to organize — directory is tidy!{RESET}", col=COL_OK)
        return
    ...
    if _USE_TUI:
        ...
        if preview:
            _summary({k:v for k,v in prev.categories.items() if v},
                     n, prev.other_files, prev.ignored_files,
    ...
    if _USE_TUI:
        ...
        with ExecutionScreen(n, target) as xs:
            ...
            for r in result.records:
                xs.log.append((r.category, r.filename,
    ...
    if log_path and getattr(args, "verbose", False):
        _dim(f"History saved: {log_path.name}")

    if getattr(args, "verbose", False):
        for r in result.records:
            dest = Path(r.destination).parent.name
            col  = cat_fg(r.category)
            print(f"  {col}{cat_icon(r.category)}{RESET}  "
                  f"{FG_BRIGHT}{r.filename:<40}{RESET}  {FG_MUTED}→{RESET}  {col}{dest}/{RESET}")

    _summary({k:v for k,v in result.categories.items() if v},
             len(result.records), result.other_files,
             result.ignored_files, elapsed, dry=False)
    ...
    if scan.found and not _QUIET:
        ...
        ok = _confirm(
            " Empty Directories ",
            body_lines=["", f"  Remove {n_empty} empty directories?", ""],
            yes=" Remove ", no=" Skip   ",
            plain_prompt=f"Remove {n_empty} empty directories?",
        )
        if ok:
...
def main() -> None:
    ...
    # ── Find subcommand by scanning ALL positionals ────────────────────────────
    # This fixes: `foldr ~/Downloads/Documents undo --id abc`
    # where first_pos = '~/Downloads/Documents' (not 'undo')
    _SUBCMDS = {"watch", "undo", "history", "config"}
    ...
    if _QUIET or args.plain or not is_tty():
        ...
        saved = get_mode()  # "tui" | "cli"

        # First-run: prefs.json doesn't exist yet → ask
    ...
    # Filter out the subcommand from path detection
    path_candidates = [a for a in raw
    ...
    if not raw_path:
        ...
        if not _QUIET:
            _box(
                f"  No path specified.\n\n"
                f"  Target:  {ACCENT+BOLD}{cwd}{RESET}\n\n"
                f"  {FG_MUTED}Tip: pass a directory — e.g.  foldr ~/Downloads{RESET}",
                title=" FOLDR ",
            )
        ok = _confirm(
            " Organize Current Directory ",
            body_lines=["", f"  Organize: {ACCENT+BOLD}{cwd.name}/{RESET}",
                        f"  {FG_MUTED}{cwd}{RESET}", ""],
            yes=" Organize ", no=" Cancel ",
            plain_prompt=f"Organize {cwd.name} (current directory)?",
        )
    ...
    if args.dedup:
        cmd_dedup(target, args.dedup, args.recursive,
                  getattr(args, "depth", None), args.preview,
